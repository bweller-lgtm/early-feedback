...
name: evaluate
description: >-
  Evaluate a product idea with synthetic user interviews and expert panels.
  Produces a scored report with persona transcripts, expert assessments, assumption
  mapping, and a validation playbook — ranked hypotheses with specific questions to
  ask real people and who to ask them. Use when the user wants to evaluate, stress-test,
  or get feedback on a product idea, pitch deck, startup concept, or project.
  Supports inline text, file paths, and directory scanning.
license: MIT
